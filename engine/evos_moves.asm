...
; not trade evolution
	...
	ld a, b
	; new evolution methods, Mateo's version
	cp EV_LEVEL
	jp z, .checkLevel
	cp EV_MAP
	jp z, .checkMapEvo
...
.checkMapEvo
	...
	jp nz, .nextEvoEntry2
	ld a, [wcfb9] ; This has to be in "a" for the evolution to work properly
	jp .asm_3adb6 ; Do evolution
.checkMoveEvo
	ld a, [hli] ; get the move number
	ld [wd0e0], a ; store it here to hang onto it
	...
	ld a, [wcfb9] ; This has to be in "a" for the evolution to work properly
	jp .asm_3adb6 ; If they did know it, do the evolution
.checkRandomEvo
...
.TyrogueDone
    ld a, [wcfb9]
    jr .asm_3adb6 ; Do first Pokemon if Def is higher
.checkItemEvo
	ld a, [hli]
